...
export default function Dashboard() {
  ...
  const quotes = { pending: 2, pendingPence: 654000, acceptedPence: 403000 };
  ...
  return (
    <>
      <div className="page-heading">
        ...
        <Link href="/demo/quotes" className="button">
          View sample quotes <ArrowUpRight size={17} />
        </Link>
      </div>
      <div className="stats-grid">
        {cards.map(({ label, value, note, Icon, status }) => (
          <Link
            href={`/demo/leads${status ? `?status=${encodeURIComponent(status)}` : ""}`}
            className="stat-card"
            key={label}
          >
            <div>
      ...
      <div className="overview-grid">
        <section className="panel recent-panel">
          <div className="panel-heading">
            ...
            <Link className="text-link" href="/demo/leads">
              View all <ArrowRight size={16} />
            </Link>
          </div>
          <LeadTable leads={leads.slice(0, 6)} basePath="/demo" />
        </section>
        <section className="panel pipeline">
          ...
          {statuses.map((s) => (
            <Link
              key={s}
              href={`/demo/leads?status=${encodeURIComponent(s)}`}
      ...
      <div className="dashboard-footnote">
        ...
        <span>Statistics cover fictional sample leads.</span>
      </div>
    </>
  );
}
